...
%     scatter(RRTState.pathvertices(:,1), RRTState.pathvertices(:,2), 'filled', 'MarkerFaceColor', 'm');
    ...
    scatter(Rover.next_wayPoint(1), Rover.next_wayPoint(2), 'filled', 'MarkerFaceColor', 'm');
    ...
    plot([Rover.pos_curr(1),Rover.pos_curr(1)+dir(1)*Rover.Radius],[Rover.pos_curr(2), Rover.pos_curr(2)+dir(2)*Rover.Radius],'LineWidth', 0.3,'Color','k');
     rectangle('Position',[Rover.pos_curr(1)-Rover.LoSRadius, Rover.pos_curr(2)-Rover.LoSRadius, 2*Rover.LoSRadius, 2*Rover.LoSRadius],'Curvature', [1,1], 'EdgeColor','w')
    % Rover.LosCircle(j,:)
    % sz = 100
